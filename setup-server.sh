...
sudo mkdir -p /var/www/webapp/issouffadika/{current,backups,temp,logs}
sudo chown -R $USER:$USER /var/www/webapp/issouffadika

# Rendre le script de déploiement exécutable
echo "🔧 Configuration des permissions..."
chmod +x /var/www/webapp/issouffadika/deploy.sh
...
sudo tee /etc/nginx/sites-available/issouffadika << EOF
server {
    listen 80;
    server_name issouffadika.com; # Remplacez par votre domaine
...
# Activer le site
sudo ln -s /etc/nginx/sites-available/issouffadika /etc/nginx/sites-enabled/
sudo nginx -t
