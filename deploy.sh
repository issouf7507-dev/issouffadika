...
# Variables
PROJECT_DIR="/var/www/webapp/issouffadika"
...
cd $TEMP_DIR
git clone https://github.com/issouf7507-dev/issouffadika.git .
...
if pm2 list | grep -q "issouffadika"; then
    pm2 restart issouffadika
else
    pm2 start npm --name "issouffadika" -- start
fi
